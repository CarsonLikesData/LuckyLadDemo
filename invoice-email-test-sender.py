...
class EmailConfig:
    ...
    def __init__(self):
        ...
        self.gmail_recipient = "dft.luckylad.test@gmail.com"  # From processor code

        # Outlook settings
        self.outlook_sender = os.environ.get("TEST_OUTLOOK_SENDER", "")
        self.outlook_password = os.environ.get("TEST_OUTLOOK_PASSWORD", "")
        self.outlook_server = os.environ.get(
            "TEST_OUTLOOK_SERVER", "outlook.office365.com"
        )
        self.outlook_recipient = os.environ.get(
            "OUTLOOK_EMAIL", ""
        )  # From processor code
        ...
        # Invoice folder
        self.invoice_folder = "synthetic_invoices\\pdf"  # Folder containing PDF invoices
...
class EmailSender:
    ...
    def __init__(self, config: EmailConfig):
        ...
        self.failed_count = 0
        self.log_file = (
            f"email_test_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        self.log_data = []
    ...
    def get_invoice_files(self):
        ...
        if not invoice_path.exists():
            raise FileNotFoundError(
                f"Invoice folder not found: {self.config.invoice_folder}"
            )

        pdf_files = list(invoice_path.glob("*.pdf"))
        if not pdf_files:
            raise FileNotFoundError(
                f"No PDF files found in: {self.config.invoice_folder}"
            )
    ...
    def run_test(self):
        ...
        while email_index < min(self.config.max_emails, len(pdf_files) * 10):
            ...
            batch_end = min(
                batch_start + self.config.batch_size, self.config.max_emails
            )
            ...
            if (
                email_index < self.config.max_emails
                and email_index < len(pdf_files) * 10
            ):
...
class OutlookSender(EmailSender):
    ...
    def __init__(self, config: EmailConfig):
        ...
        credentials = Credentials(
            username=self.config.outlook_sender, password=self.config.outlook_password
        )

        config_obj = Configuration(
            server=self.config.outlook_server, credentials=credentials
        )
    ...
    def send_email(self, email_content, pdf_file):
        ...
        with open(pdf_file, "rb") as f:
            content = f.read()
            attachment = FileAttachment(name=pdf_file.name, content=content)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Send test emails with invoice attachments"
    )
    parser.add_argument(
        "--method",
