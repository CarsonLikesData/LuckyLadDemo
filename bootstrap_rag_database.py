...
def bootstrap_database(sample_dir, force=False):
    ...
    for i, file_path in enumerate(pdf_files, 1):
        ...
        if json_metadata:
            ...
            if success:
                print(f"Successfully added {os.path.basename(file_path)} to RAG database (from JSON).")
                successful += 1
            else:
                print(f"Failed to add {os.path.basename(file_path)} to RAG database (from JSON).")
        else:
            # Process the PDF
            extracted_data = process_sample_invoice(file_path)
            ...
            if extracted_data:
                ...
                if success:
                    print(f"Successfully added {os.path.basename(file_path)} to RAG database.")
                    successful += 1
                else:
                    print(f"Failed to add {os.path.basename(file_path)} to RAG database.")
    
    print(f"\nBootstrap complete. Added {successful}/{len(pdf_files)} invoices to RAG database.")

def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Bootstrap RAG database with sample invoices")
    parser.add_argument("--dir", default=SAMPLE_INVOICES_DIR, help=f"Directory containing sample invoices (default: {SAMPLE_INVOICES_DIR})")
    parser.add_argument("--force", action="store_true", help="Force overwrite of existing database")
